...
循環参照（ループ）は禁止し、チェーンは必要に応じて切ってよい（品質や負担で `ja` 直に戻すのは可）。

- ルート: `ja`
- 第1段階（主要）: `ja -> en` / `ja -> zh-Hans` / `ja -> zh-Hant` / `ja -> ko`
- 第1段階（負担軽減）: `ja -> es` / `ja -> pt` / `ja -> fr` / `ja -> de` / `ja -> it` / `ja -> ru` / `ja -> pl` / `ja -> fil`
- 第2段階（英語派生）: `en -> fr` / `en -> de` / `en -> it` / `en -> es` / `en -> pt` / `en -> ru` / `en -> pl`
- 第2段階（地域派生）: `en -> en-GB` / `en -> en-AU` / `en -> en-IE` / `es -> es-419` / `pt -> pt-BR` / `fr -> fr-CA`
- 第2段階（北欧）: `en -> no` / `en -> sv` / `en -> fi`

メモ:

- `zh-Hans` と `zh-Hant` の関係（片方から変換するか、両方 `ja` から作るか）は別途決める。
- 1つの言語は「派生元を1つだけ」に固定し、`ja` に戻す以外の切り替えはしない（ループ防止）。
